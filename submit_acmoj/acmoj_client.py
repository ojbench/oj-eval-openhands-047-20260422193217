...
def main():
    ...
    # Code submission sub-command (submit raw file content)
    submit_code_parser = subparsers.add_parser("submit-code", help="Submit raw code content")
    submit_code_parser.add_argument("--problem-id", type=int, required=True, help="Problem ID")
    submit_code_parser.add_argument("--file", type=str, default="src.hpp", help="Path to code file to submit (default: src.hpp)")
    ...
    if args.command == "submit":
        ...
        try:
            ...
            print(json.dumps({"status": "error", "message": f"Failed to read file {args.file}: {e}"}))
            return
        result = client.submit_code(args.problem_id, code_content, args.language)
